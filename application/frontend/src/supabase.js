...
// guessing.
//
// The publishable key (`sb_publishable_…`, which replaced the legacy `anon` JWT) is public by
// design: it ships in this bundle either way, and this project's Data API is disabled with RLS
// deny-by-default, so it opens nothing. The SECRET key is a different animal — admin-grade,
// browser-forbidden by Supabase itself, and it lives only in application/config.py.
...
export const authConfigured =
  Boolean(url && publishableKey) && url !== PLACEHOLDER && publishableKey !== PLACEHOLDER;
...
if (!authConfigured) {
  console.error(
    ...
    `  VITE_SUPABASE_PUBLISHABLE_KEY: ${publishableKey ? '(set)' : '(unset)'}\n` +
...
export const supabase = !authConfigured ? null : createClient(url, publishableKey, {
  auth: {
