...
**Last reviewed:** 2026-07-11 (§2 Player-News Collector — the RSS aggregation half of the ROS AI-interpretation layer; `player_news` entity + `fetchers/news.py`)
...
└── application/
    ...
    │   │   ├── news.py             # ✅ built — daily player-news RSS collector (§2 ROS AI-layer aggregation half)
    ...
    │   └── snapshots/              # time-series parquet (gitignored)
            ...
            ├── news/
            │   └── player_news.parquet              # §2 aggregation half: current NFL player news, de-duped + resolved to sleeper_player_id + source-attributed; one row per (item × player); append-only-of-new by item_id; compact items only (no article bodies) — url + collected_at = Wayback recall path
            ├── projections/
    ...
    ├── ai/                         # the AI layer (§7 Manager Dossiers Phase B) — the project's FIRST AI-layer code; distinct from the deterministic polars transforms. Parquet I/O still via data_layer; the Anthropic call is external (like a fetcher's HTTP)
        ├── client.py                  # ✅ built — the isolation seam: api_available() key gate (locked on absent/placeholder/non-sk-ant) + generate_dossier() (THE swap point: synchronous messages.create, Haiku 4.5, no thinking/effort, tolerant json.loads NOT messages.parse; a future Batch path swaps here only)
        ├── dossier_prompt.py          # ✅ built — pure prompt construction: system_prompt (fixed 7-key JSON schema + tendencies-not-verdicts guardrails) + user_prompt (blindspot framing for is_primary / exploitable-edge for opponents) + zero_signal_dossier (hardcoded "no intel")
        ├── write_manager_dossiers.py  # ✅ built — the writer (compute/run/--season/--force): per manager, zero comparable leagues ⇒ hardcoded (no API) else prompt→generate→validate schema; synchronous sequential; run-once-per-season guard; key-gate clean exit → derived/manager_dossiers
        └── check_manager_dossiers.py  # ✅ built — dossiers gate (internal consistency, no API, reads persisted only): coverage + schema completeness + depth-echo-matches-features + zero-signal honesty (exit 0)
    ├── config.example.py
    └── requirements.txt
```
...
| NFL news (RSS) | snapshots/ only (`player_news`) | Public RSS from a national NFL feed registry (`fetchers/news.py`, `feedparser`). Live now-stream: point-in-time historical news is a trap, so it's snapshotted forward (the value time-series exists only if we bank it, like leaguelogs). No API key. The §2 ROS AI-interpretation layer's aggregation half; the on-demand AI synthesis reads it. |
...
| fetchers/news.py (collector) | snapshots/news/ | player_news.parquet — **the §2 ROS AI-interpretation layer's aggregation half.** A live, scheduled RSS collector banking current NFL player news as a de-duplicated, player-resolved, source-attributed time-series the future on-demand AI synthesis reads. **One growing file** (like leaguelogs market_values); grain = **one row per (news item × resolved player)**. Resolves item text to `sleeper_player_id` by **exact full-name match** against the **967 active-rostered** (`team`-not-null) skill players — that universe has **0 name collisions** (a high-precision key), with defensive **team-mention disambiguation** for the rare collision the daily-mutating registry will surface (`match_confidence` exact_full/disambiguated; ambiguous → skipped, never guessed — law 2). Stores **compact items only** (headline / summary / url / published_at / **collected_at** / source / source_type / season / week / match cols) — **never the article body**: url + collected_at are the Wayback recall path, and it sidesteps copyright/ToS. Nationals-only registry now (`source_type` stored so team beats drop in later with no schema change). The writer (`write_player_news`) is **append-only-of-new by `item_id`** (sha1(url|player_id)) — a re-poll adds nothing (idempotent), and cross-feed reprints collapse. **Reliability** (leaguelogs lesson): `_get_feed` timeout + backoff retry, **per-feed isolation** (one dead feed never kills the run), **incremental per-feed persistence**. Live-acquired like manager_activity — the **forward pipeline**, deliberately NOT tied to the frozen-2025 league. Scheduled by launchd at 5am ET (`com.fantasyai.news-snapshot`). Decision home: the §2 ROS hold/upside read (via the on-demand synthesis, next build) |
...
- `news.py` - built; **player-news collector** (the §2 ROS AI-interpretation layer's aggregation half) — `snapshot`/`feeds`/`resolve-test`/`check`. A live RSS collector over a national NFL feed registry (`feedparser` + `_get_feed` timeout/backoff; per-feed isolation; incremental per-feed writes), resolving items to the 967 active-rostered skill players (exact full-name match, 0 collisions; defensive team disambiguation) → `player_news` entity. Compact items only (headline/summary/url + `collected_at` — never article bodies). Scheduled by launchd at 5am ET. Live-acquired like manager_activity (the forward pipeline, not tied to frozen-2025). The on-demand AI **synthesis** over this feed is the next build
