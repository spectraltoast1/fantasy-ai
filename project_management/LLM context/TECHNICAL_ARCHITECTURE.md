...
- `compute_bracket_sim.py` — **the bracket-math half of the Posture read (DECISION_READS §5): a Monte
  ...
  `magic_wins`. **Playoff config** (`REG_SEASON_END_WEEK=15`, `PLAYOFF_TEAMS=6`) is a **documented
  latent** — inferred from the 2025 schedule (wks 1–15 pair all ten teams; wk16 splits into a two-bye
  bracket ⇒ 6), because the league's real settings aren't in the frozen store (the `/league` fetch that
  built lineup_slots didn't persist them — persisting them is a follow-up, like the scoring_settings
  recompute latent). Tall over as_of_week. **Documented simplifications:** independence across starters
