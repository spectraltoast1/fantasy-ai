...
- ~~**Standard lineup shape.**~~ **RESOLVED (any-league piece 2); VALIDATED ON REAL SHAPES (Session 3d).**
  ...
  `position_pools`.)*
- ~~**Division playoff seeding is synthetic-gated only.**~~ **RESOLVED + VALIDATED ON REAL DIVISION LEAGUES
  (Session 3d, any-league piece 3).** `compute_bracket_sim._seed_table` seeds division winners ahead of
  wildcards when a roster→division map is present. The 3d catalog found the real gap: 3a's teams fetcher
  dropped `settings.division` (it lives on the rosters endpoint), so `_division_map` returned None and all
  harvested division leagues were silently seeded FLAT. Fixed: `fetch_teams` now persists `division`, and a
  bounded re-harvest (`corpus/backfill_division.py`) additively backfilled it onto the **14 generalization
  division leagues** (2–4 divisions each), activating division-aware seeding — proven to lift a losing-record
  division winner past a higher-record wildcard, mass==slot-count preserved, deterministic. **⚠ Report-only
  latent:** the **11 matched division leagues remain flat-seeded** — activating them would move the frozen
  3b/3c tuning spine (out of 3d scope); `_division_map` returns None for their untouched teams. Certified on
  the `never_tune` generalization stratum; gated by `corpus/check_spine.py` on real division brackets.
