...
footnote. **Fold-in (merged Session 2 into 1.7):** the §2 ADP anchor is live post-merge (Session 2's
`read_adp_points_curve(holdout=season)` reader) and gains a missing anchor-consumption gate in
`backtest_ros_player_band` (below). — Prior — **Backend — NFL Substrate Backfill (Improvement-Loop Session 2)** — the
technical facts worth carrying: (1) a NEW derived-entity SHAPE — **`adp_points_curve/holdout_{S}.parquet`**:
the §2 ADP anchor curve is now persisted **per held-out target season** (fit on every season EXCEPT S, +
`holdout_season`/`train_seasons` provenance), replacing the single season-agnostic file. `data_layer`'s
`read/write/adp_points_curve_exists` take a `holdout` arg, `compute_ros_player_band` reads `holdout=season`,
and the new **`check_adp_curve_leakage.py`** hard-gates it (a multi-season corpus that fit the anchor on the
season it grades would be silently optimistic). (2) **`ros_player_band` now spans the FULL projected season**
— the wk-4 roster freeze retired (the band is scoring-scoped/roster-free and no longer reads `join_season`, its
last roster-path dependency); the live 2026 AI anchor is protected by **pinning `write_ros_synthesis._read_anchor`**
to the league-view's freeze as-of (byte-identical served synthesis, proven). (3) **`_scoring.standard_scoring(key)`**
(the inverse of `scoring_profile`) + a `--scoring-key` on `compute_projection_consensus`/`compute_ros_player_band`
+ a `build_substrate.py` driver build the NFL-global substrate for `{ppr,half}×2020–2025` independently of any
league — the FIRST explicit-scoring-key callers (historical seasons have no is_mine league to resolve). Report-
don't-tune finding: BAND_Z=0.55 generalizes across 2020–2025 (2025 is not the outlier), SKEW_GAIN=1.5 is fragile
(helps 2020/21/22/25, hurts 2023/24) — constants left exactly as-is. — Prior — **Backend — Gate Repair
(Improvement-Loop Session 1.6)** — two technical decisions worth carrying: (1) **`data_layer._active_league_any`** — a season-robust sibling of
...
**Retired reads — `team_leakage` and `team_form`:** removed in the Session 1.5 scope-correction. `team_leakage` graded start/sit against **realized points** (a design-law-1 violation — it coached the spike-week error); `team_form` was an EWMA recency trend (noise). Both were fully orphaned. The *process*-graded successor to leakage — grading a start/sit against the projection prior knowable at decision time (`projection_consensus`) — is legitimate future work, not built. See `DECISION_READS.md` → Retired reads.

---

## Front-end shaping — portability & latent assumptions

The heaviest analytics live in **Python transforms** that pre-compute parquet
(`compute_production_vor.py`, `compute_true_rank.py`, `compute_positional_depth.py`,
`compute_player_signal.py`, … → `derived/`); `src/queries.js` is the thin seam that reads them
and the lighter shaping it still computes inline (all-play, spectrums, the optimal-lineup
projection). Views stay pure renderers. Three classes of portability to keep straight:
...
the parquets (`season`, `teams`, `lineup_slots`), not code. Week-windowed reads derive `n`
dynamically and widen as weeks append; a new standard Sleeper league means new parquets + a
transform re-run, not new logic.

**Season-replay seam (`as_of_week`):** the derived parquets are **tall** — one slice per
as-of week N. A global "As of" **week selector** lives in the App shell (`App.jsx`): `App.jsx`
owns the `asOfWeek` state and passes it to the views, so one active week applies across all
surfaces and persists across tab switches. It threads through `queries.js` via two helpers —
`asOfSlice(table, n)` parameterises the derived reads' inner `max(as_of_week)` (pick week N's
slice; `n == null` ⇒ latest, the default), and `weekCutoff(n)` adds `WHERE week ≤ N` to the
still-in-JS SQL reads (standings/all-play, players, matchups). `loadWeeks()` feeds the dropdown
(weeks 1..latest; default = latest, travels back only). The selector drives the readiness gate
(`weeksElapsed = asOfWeek`) and replaced the temporary `?weeksOverride` QA param. This is the
one place "which week am I viewing" lives behind the seam.

**Latent assumptions (won't self-correct — silent wrong output, not an error):**
- ~~**Standard lineup shape.**~~ **RESOLVED (any-league piece 2); VALIDATED ON REAL SHAPES (Session 3d).**
  `compute_production_vor.py`'s replacement pools derive from the league's `lineup_slots` via shared
  `_analytics.position_pools` (superflex pools QB with the flex; standard is byte-identical). The 3d
  generalization pass ran this over **21 real superflex-classed leagues**: `position_pools` correctly merges
  QB into the `SUPER_FLEX` pool on the **15 with a real SUPER_FLEX slot**, and correctly keeps QB its own
  pool on the **6 multi-dedicated-QB (2QB/3QB)** leagues (it keys off actual slot eligibility, not the coarse
  manifest label). All exotic lineup slot codes across the 48 gen leagues are recognized (0 silently dropped).
  Gated by `backtest_roster_shape.py` + `corpus/check_spine.py` (generalization arm). *(The remaining shape
  latent is **TE-premium lineup shape** beyond scoring — but TE-premium **scoring** is handled by the
  any-league scoring engine; a dedicated 2-TE slot config already flows through `lineup_slots` +
  `position_pools`.)*
- ~~**Division playoff seeding is synthetic-gated only.**~~ **RESOLVED + VALIDATED ON ALL 25 REAL CORPUS
  DIVISION LEAGUES (Sessions 3d + 3e, any-league piece 3).** `compute_bracket_sim._seed_table` seeds division
  winners ahead of wildcards when a roster→division map is present. The 3d catalog found the real gap: 3a's
  teams fetcher dropped `settings.division` (it lives on the rosters endpoint), so `_division_map` returned
  None and all harvested division leagues were silently seeded FLAT. Fixed: `fetch_teams` now persists
  `division`, and `corpus/backfill_division.py` additively backfilled it onto the teams entity —
  **Session 3d** on the 14 generalization division leagues, **Session 3e** on the 11 matched division leagues
  (`--stratum matched`). All 25 now seed division-aware — proven to lift a losing-record division winner past a
  higher-record wildcard, mass==slot-count preserved, deterministic. 3e was bounded + proven: activating the 11
  moved **only their `bracket_odds`** (baseline SHA diff: exactly 11 files changed of 1110; a flat recompute
  reproduces their 3b `bracket_odds` byte-for-byte → division is the sole delta; no input drift). Gated by
  `corpus/check_spine.py` (mass == slot count on real division brackets, both strata).
...
constants near the logic that uses them. `compute_player_signal.py` owns its signal set —
`SHRINK_K`, `MIN_GAMES`, `SPIKE_BAND`/`STICKY_BAND`, `POS_MEAN_MIN_OPP`, `OPP_HALF_LIFE_WK`,
`DIRECTION_HALF_LIFE_WK`/`DIRECTION_BAND`. Candidates to become league/user-configurable one
day — document the seam, don't build the config now (premature flexibility is its own debt).
Lift to a shared config object when that seam is built. (Note: `MIN_GAMES` now lives in
exactly **one** place — `compute_player_signal.py` (=3, the per-player games gate). Its former
duplicates in the retired `compute_team_leakage.py` (=2) and in `queries.js`'s deleted inline
reads (=2) are gone with the `team_form`/`team_leakage` retirement.)
...
└── application/
    ...
    │   └── public/data/             #   symlinks → season_2025 + teams_2025 + lineup_slots_2025 + league_settings_2025 + player_signal_2025 + production_vor_2025 + market_vor_2025 + ros_synthesis_2026 parquet (gitignored)
    ├── data/
    ...
    │   └── snapshots/              # time-series parquet (gitignored)
            ├── derived/                                 # pre-computed analytics (compute_*.py output); player_signal + production_vor + true_rank + positional_depth are tall, grain (season, as_of_week, entity); projection_consensus is per (season, week, player)
            ...
            │   ├── adp_points_curve/holdout_{S}.parquet # §2 anchor curve, ONE per held-out target season S (Session 2): per (position, pos_ecr_rank) the historical realized-points floor/center/ceiling (P10/P50/P90, rolling-window + isotonic non-increasing), fit by compute_adp_points_curve.py over every season EXCEPT S (+ holdout_season/train_seasons provenance) so a season-S band's anchor never saw S. Read via read_adp_points_curve(holdout=S); leak-gated by check_adp_curve_leakage.py. (Was a single season-agnostic file — a corpus leak.)
            ...
            └── nfl_sleeper_weekly_joined/
                └── league/<league_id>/                  # L0 league-keyed (Session 3a); the 5 measurement reads (production_vor/true_rank/positional_depth/bracket_odds/player_signal) computed here for the 221 matched corpus leagues by corpus/compute_spine.py (Session 3b) — one dir per league. Stage-B B2 also lands the league-scoped `schedule`, `manager_features`, and `manager_dossiers` here per demo slice (via corpus/compute_demo_slices.py)
    ...
    │       └── nflreadpy/
    │           ├── nfl_stats_2025.parquet  # per player-week, weeks 1-18. Carries the ff_opportunity `*_exp` expected-points components (the §1 Quality basis, re-scored at the consumption layer) + redzone_touches (PBP companion). `xtd` (the old TD-proxy) fully retired — dropped from the fetcher output AND purged from the stale 2020–24 parquets (`corpus/retire_xtd.py`, Corpus-cleanup), so no season carries it and the six-season schema is uniform (166 cols). **All six seasons (2020–2025) now carry the 14 `*_exp` components** — 2020–24 were **additively backfilled** in Session 3c (`nfl_stats.backfill_exp(year)`: append `*_exp` only, every pre-existing column byte-identical — NOT a re-pull, to avoid moving-source drift on the frozen corpus), so §1 Quality is out-of-sample across the corpus, not TEST-only. 2020–2024 were earlier backfilled for realized points too (the §2 ADP curve source)
    │           └── adp_preseason.parquet   # historical preseason ADP (FantasyPros via load_ff_rankings): one tall file, season a column; per (season, player) ecr/best/worst/sd + pos_ecr_rank, latest full-board pre-kickoff redraft-overall snapshot/season, id-bridged fantasypros_id→sleeper. The §2 preseason-anchor source (fetchers/adp.py)
            └── sleeper/
                └── <season>/
                    └── league/<league_id>/           # L0 league-keyed raw (Session 3a) — one dir per league-season
                        ...
                        └── ... # matchups_week_NN + transactions_week_NN parquet for each week of the season
    ...
    ├── transforms/ # one Python script per join/transform
        ...
        ├── compute_adp_points_curve.py      # ✅ built — §2 preseason-anchor curve: per (position, pos_ecr_rank) historical realized-points floor/center/ceiling (P10/P50/P90, rolling ±3-rank window, isotonic non-increasing) over prior seasons (target held out = leak-free) → derived/adp_points_curve/holdout_{S}.parquet, ONE per held-out season (--holdout S | --all) with holdout_season/train_seasons provenance. Realized points read from nfl_stats (no transform hits nflreadpy)
        ├── check_adp_curve_leakage.py       # ✅ built — HARD leak gate (Session 2): for every fittable season S, asserts holdout_S carries holdout_season==S, S ∉ train_seasons, train == the season complement, AND the persisted curve == recompute(holdout=S). Stops a corpus from grading §2 on a season its anchor was fit on (exit 0)
        ├── build_substrate.py               # ✅ built — Session 2 driver: builds the NFL-global substrate (projection_consensus → ros_player_band, in order) for {ppr,half}×{2020..2025} via _scoring.standard_scoring(key) + the --scoring-key path. Matched-stratum keys only; generalization's custom keys deferred to Session 3
        ...
        ├── compute_manager_features.py      # ✅ built — Manager Dossiers §7 Phase A: cross-league behavioral features per manager (owner_id) from manager_activity → derived/league/<id>/manager_features. Pure manager_features (injected constants); every league manager gets a row incl. zero-signal (null features). League-scoped in B2 (compute/run take `*, league_id=None`, default is_mine)
    ...
    ├── ai/                         # the AI layer — distinct from the deterministic polars transforms. Parquet I/O still via data_layer; the Anthropic call is external (like a fetcher's HTTP). Three reads now: §7 Manager Dossiers (Phase B) + §2 Team News Dossier (news pipeline Stage B) + §2 ROS Synthesis (the interpretation half of §2)
        ...
        ├── write_manager_dossiers.py  # ✅ built — §7 writer (compute/run/--season/--league-id/--force): an included AI run (ungated); per manager, zero comparable leagues ⇒ hardcoded (no API) else prompt→generate→validate schema; synchronous sequential; run-once-per-season guard → derived/league/<id>/manager_dossiers. League-scoped in B2 (`*, league_id=None`, default is_mine)
        ├── check_manager_dossiers.py  # ✅ built — §7 dossiers gate (internal consistency, no API, reads persisted only): coverage + schema completeness + depth-echo-matches-features + zero-signal honesty (exit 0). League-scoped in B2 (run takes `*, league_id=None`)
...
- **League-scoped** — `+ league_id`: `player_signal`, `production_vor`, `market_vor`, `ros_league_view`,
  `ros_synthesis`, `true_rank`, `positional_depth`, `bracket_odds`,
...
- **League-scoped RAW/JOIN** (Session 3a) — `+ league_id`: the fetched Sleeper layer + the join,
  previously season-keyed only (the deferred half of L0): `sleeper_matchups`, `sleeper_transactions`,
  `teams`, `roster_positions`, `lineup_slots`, `league_settings`, and `join_season` (+ `remainders`).
...
- internal
| nfl_sleeper_weekly_joined transform | snapshots/nfl_sleeper_weekly_joined/league/{league_id}/ | Joined output — one file per league-season (season_{season}.parquet), each week appended with a (season, week) dedup guard; corpus joins also carry `is_two_way`. League-keyed since Session 3a (was season-keyed) |
| derive_lineup_slots transform | snapshots/sleeper/{season}/league/{league_id}/ | lineup_slots_{season}.parquet — starting skill-slot requirements (slot, count, eligible) derived from the league's raw roster_positions; declares the QB/RB/WR/TE + FLEX config so the front-end "perfect lineup" / efficiency calc is exact, not inferred. League-keyed since Session 3a |
...
- `nfl_stats.py` - backfill + refresh modes, polars, player ID map. Joins the ff_opportunity `*_exp` expected-points components (`_load_ff_opportunity`, the §1 Quality basis) + `redzone_touches` (`_load_redzone_touches`) onto each player-week; the old `xtd` td_prob proxy is retired. **Additive-backfill mode (Session 3c):** `precheck_exp()` (schema-honesty pre-check — confirm ff_opportunity serves all 14 `EXP_COMPONENT_COLS` populated before any write) + `backfill_exp(year)` / `backfill_exp_all()` append the `*_exp` components onto an EXISTING season parquet WITHOUT re-pulling the rest (a byte-identical additive backfill, not a rebuild — the Session-2 pattern for closing a stale-substrate gap without moving-source drift); CLI `precheck-exp` / `backfill-exp <year|all>`
...
Cache files are JSON (league/user/roster state) or parquet (players registry). Snapshot files are parquet partitioned by season and (since Session 3a) by league: snapshots/sleeper/<year>/league/<league_id>/
...
- **Season-replay `as_of_week` dimension (all three derived transforms).** Each
  ...
  is per-analytic, decoupled from the cutoff, by the stationarity principle:**
  player-opportunity reads through an injected EWMA half-life (`OPP_HALF_LIFE_WK`, shared
  `_weighted_rates`) — backtest-tuned and shipping **cumulative** (the 2025 sweep showed
  recency hurts rest-of-season MAE). `data_layer.read_player_signal` takes an optional
  `as_of_week` (default = latest), and `queries.js` carries a default-latest guard, so
  existing callers and the front end are unchanged until the Session-B week selector lands.

- **Derived-analytics transforms — SOLID shape (per principle #9).** Each analytic (e.g.
  `compute_player_signal`, `compute_production_vor`) is a pure function that **receives its tuning
  constants as injected keyword args** — `compute(season)` is the composition root that owns the module
  constants and passes them down (DIP: the pure logic depends on parameters, not globals, so it tests in
  isolation at any parameterisation). The analytics live in **Python** (not the JS seam) because a Python
  server is the eventual architecture, so the migration becomes "transform → API serves same parquet"
  rather than "rewrite JS math in Python." Re-run with `--season` after a join refresh. Shared numeric
  helpers (`round1`, `mean`, `median`, the league-relative `spectrum_positions` normaliser — the Python
  mirror of the front-end's old `attachSpectrumPos` — `pearson` for the point-correlation refinement,
  and the slot-aware greedy lineup engine `expand_slots`/`optimal_lineup`) live once in
...
- `compute_true_rank.py` — **the first league-level read that consumes the substrate (DECISION_READS
  ...
  `optimal_lineup`) lives in `_analytics`** (pure, points-agnostic — the "shared helper has one home"
  move), consumed by True Rank, Production VOR and the bracket sim. Record-independent (no wins read);
  slot-aware so a capped-position stud
