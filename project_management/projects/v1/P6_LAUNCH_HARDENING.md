...
- **(S2c audit, 2026-08-11) Two Fly machines, therefore two connection pools** against a **free-tier**
  Postgres with a modest connection cap. Probably fine at default pool sizes — but Supabase refusing
  connections is the one failure that does not self-heal, so measure it rather than assume it. The machine
  count itself was assumed once already and was wrong.
