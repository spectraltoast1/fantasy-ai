...
**Created:** 2026-07-26 · **Updated:** 2026-08-14 — **S4a SHIPPED + ENDORSED**; S4 is now **S4a/S4b/S4c/S4d** (the queue and the connect flow are separate sessions) · **Status:** Active — **S4b next, briefed** · **Track:** Critical spine — **the biggest single block** · **Depends on:** P0 (keyed reads + viewer-as-data), P2 (the ingestion pipeline it automates) · **Est:** 10 sessions (S4 became four) · **Session docs:** `sessions/v1/P5-Self_Serve/`
...
| **S4b — The job queue** — *briefed 2026-08-14* | Work reaches the worker without a human | **The catalog-drift guard first**, then a `jobs` table **in `api/auth_schema.sql`** (NOT `serve/schema.sql`, which `--load` DROPs), a `FOR UPDATE SKIP LOCKED` lease with an expiry, and the worker's `sleep infinity` CMD replaced by a leasing loop. **`validating` is a SLOT — S5 owns preflight.** The forcing function is not latency: the API has no polars and the worker has no `http_service`, so the two machines cannot talk | A job row inserted **by hand** is leased, run to `ready`, and lands in Postgres **with nobody touching `fly ssh` after the insert**; states readable from the table alone; a killed job's lease expires and leaves **no half-built league**; `rejected` distinguishable from `failed`. → `sessions/v1/P5-Self_Serve/SESSION_P5_S4B_JOB_QUEUE.md` |
| **S4c — The connect flow + identity** | A user can ask for their league, and the app learns who they are | `POST /api/connect` + `GET /api/connect/{id}` (both enqueue/read only — the queue is S4b's); **identity ACQUISITION** — S2 settled the *model* (`user_leagues.roster_id`), not how the row gets written: `api/routes.py:102` says ownership is *"written by an operator rather than inferred from a sign-in"*, so a self-serve user signs up and reaches nothing until Will types a row; the progress screen; `scripts/users.py --delete`; **basic rate limiting** on connect (it hits Sleeper per request); **S4a finding F** — `reads.resolve_viewer` falls back to matching `MY_USERNAME` against `teams.owner_name`, so the owner's handle appearing in a connected league silently highlights a stranger's roster as "you" (**display-integrity inside a league the caller may already read — NOT a cross-user leak**; do not over-scope) | A league is onboarded through the **real** flow end to end, with progress visible and a clean re-submit, and the ownership row is written by the **user claiming their own league**, not by an operator |
...
- **S4 SPLIT into S4a / S4b / S4c / S4d — SETTLED (Will, 2026-08-14; S4b split again the same day once `onboard_league` existed).** S4 as written was a queue **plus** a
