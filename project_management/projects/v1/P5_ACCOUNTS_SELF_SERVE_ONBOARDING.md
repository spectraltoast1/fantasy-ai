...
**Created:** 2026-07-26 · **Updated:** 2026-08-14 — **S4 split into S4a / S4b / S4c** (Will), and the catalog wall re-scoped: it is three laptop-owned writers, not `write_leagues` alone · **Status:** Active — **S4a next, briefed** · **Track:** Critical spine — **the biggest single block** · **Depends on:** P0 (keyed reads + viewer-as-data), P2 (the ingestion pipeline it automates) · **Est:** 9 sessions (S4 became three) · **Session docs:** `sessions/v1/P5-Self_Serve/`
...
| **S4a — The cold onboard + the catalog a connected league lives in** — *briefed 2026-08-14* | The store can accept a league it has never seen | A **third, append-shaped catalog artifact** for connected leagues + its per-league writer (**worker-owned**; the ADR's classification goes 11 → 12); `_resolve_scoring_key` derived from the league's **own** settings; a real cold-onboard entry point promoted from `bench_cold_league`'s chain | One real, played, **2025** league — cold by an `assert_cold` extended to every catalog — is onboarded **end to end on `fantasy-ai-worker`, from its own volume, committing to production Postgres**, and `GET /api/leagues` returns it to a granted account and to nobody else while signed-out still returns exactly the demo. A re-onboard is a clean no-op. → `sessions/v1/P5-Self_Serve/SESSION_P5_S4A_COLD_ONBOARD_AND_CATALOG.md` |
| **S4b — The job queue + connect flow + identity** | A user can ask for their league, and the app learns who they are | A Postgres `jobs` table (no new infra — transactional Postgres exists and the job count is in the dozens); the worker leases one job at a time; states `queued → validating → fetching → building → loading → ready \| rejected \| failed`; `POST /api/connect` + a progress screen. **Plus identity ACQUISITION** — S2 settled the *model* (`user_leagues.roster_id`) and not how the row gets written: `api/routes.py:102` says ownership is *"written by an operator rather than inferred from a sign-in"*, so a self-serve user signs up and reaches nothing until Will types a row. Plus `scripts/users.py --delete` (account lifecycle) | A league already in the demo slate is onboarded through the **real** flow end to end, with progress visible and a clean re-submit — and the ownership row is written by the **user claiming their own league**, not by an operator |
| **S4c — The weekly cadence** | Every connected league advances each week, unattended | The refresh must **enumerate connected leagues from the ownership table** instead of one hardcoded `LEAGUE_ID`, and **take the queue's lease** so two machines never work the same league. Then **gut `.github/workflows/weekly_refresh.yml` to a pure trigger**: keep the Tue+Wed crons and the catch-up idempotency (GitHub is the better scheduler — free, reliable, already trusted for the collectors), lose all pipeline logic plus `MY_USERNAME`/`LEAGUE_ID`, which **retires** the hardcoded-identity bug rather than relocating it. Decide what the Action actually pokes — **the worker has no `http_service`** (`flyctl` from the Action is the obvious answer) | **Every connected league advances a week unattended, executing ON THE WORKER.** Retire the workflow's pipeline logic only once S4c's version is proven — though nothing is lost either way: it has **NEVER worked** (scaffolded deliberately in P2/S2 as bones for this session), and since the Aug 1 season rollover it fails at `shared/league_resolver.py:59` because its hardcoded `LEAGUE_ID` is LoRP **2025** and the resolver now asks Sleeper for **2026**. **Week-1 critical — this is what stops the app being frozen in-season** |
...
- **S4 SPLIT into S4a / S4b / S4c — SETTLED (Will, 2026-08-14).** S4 as written was a queue **plus** a
...
- **Identity mapping — HALF CLOSED, reassigned to S4b (2026-08-13; re-pointed at S4b 2026-08-14).** S2 settled the **model**: the viewer
  ...
  Sleeper username to an app user is **connect-flow work → S4b**.
